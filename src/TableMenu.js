...
export default class TableMenu extends Component {
                    ...
                    <View style={{ flex: 1, flexDirection: 'column', padding: 6 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'flex-start' }}>
                            ...
                            <TextInput
                                ...
                                style={{ backgroundColor: '#dae0e5',padding:10 }}
    ...
    renderTotal() {
        ...
        this.state.services.forEach(s => {
            s.products.forEach(p => {
                if (!p.isTasting) {
                  taxes += ((p.tax || 0) * parseInt(p.quantity > 0 ? p.quantity : 1));

                    subtotal += ((p.price || 0) * parseInt(p.quantity > 0 ? p.quantity : 1));
                }
            });
        });
...
let totals=subtotal+taxes
        return (<View style={{
            ...
            <View>
                <Text>Total: {totals.toFixed(2)}$</Text>
            </View>
        </View>
        );
    }
